...
| Clause | Requirement | Site | Test |
|---|---|---|---|
| 3.1 | The reading MUST be a CBOR integer or a decimal fraction whose mantissa is an integer or a bignum. | [decode.ts:206](../src/codec/decode.ts#L206), [:211](../src/codec/decode.ts#L211), [:218](../src/codec/decode.ts#L218), [:223](../src/codec/decode.ts#L223) — `ERR_VALUE_TYPE` | `rejection.test.ts` — `D9ACDC826161 04`, `D9ACDC82F604`, `D9ACDC82C4812004`, `D9ACDC82C48261610504`, `D9ACDC82C48220616104` |
...
| 3.2 | Decoders MUST reduce the fraction to its lowest terms, so that `[-2, 4]` and `[-1, 2]` are the same exponent. | [unit.ts:88](../src/model/unit.ts#L88) — `unitExponent` divides by the greatest common divisor on construction | `rejection.test.ts` — *rejects a rational exponent that is not in lowest terms, and reduces it leniently*; `model/unit.test.ts` |
| 3.2 | An encoder MUST write a single named unit in the named form, never as a one-element array. | [encode.ts](../src/codec/encode.ts) never writes one; [decode.ts:311](../src/codec/decode.ts#L311) refuses to read one in strict mode — `ERR_UNIT_SINGLE_AS_PRODUCT` | `rejection.test.ts` — *rejects a single named unit written as a one-element product* |
...
## 9. Where this implementation is stricter than the specification

Four spellings are well-formed under the document and refused by the strict
decoder. Each is a second way of writing something the format can already
write, which Section 6 does not permit — the encoding has to be a function of
the reading alone, and two spellings would make it a function of the encoder
too. **Lenient mode reads all four and normalises them**, which is exactly what
the specification requires of a decoder; strict mode is the additional promise
that what was accepted is what a conforming encoder wrote, and it is that
promise a signature over the bytes rests on.

| Spelling | Code | Why it is refused |
|---|---|---|
| `4([0, 5])` where `5` would do | `ERR_CBOR_NON_PREFERRED` | RFC 8949 Section 4.2.1 |
| A single named unit as a one-element product | `ERR_UNIT_SINGLE_AS_PRODUCT` | Section 3.2 requires the encoder to write the named form |
| A prefix of 0 with nothing after it | `ERR_PREFIX_REDUNDANT` | Section 3.3 requires it only when an uncertainty follows |
| An uncertainty map holding nothing but a magnitude | `ERR_UNCERTAINTY_REDUNDANT_MAP` | Section 3.4 calls the bare number the compact form |
| A rational exponent not in lowest terms | `ERR_UNIT_EXPONENT_NOT_REDUCED` | Section 3.2 requires the reduced form. The document's MUST is that decoders *reduce*, which lenient mode does; refusing the unreduced spelling is this implementation's choice, made for the reason above |

The last of these was found by the fuzz suite during WP7, through the property
that a signature depends on: strict mode used to accept `[20, 2]` and write
`10` back, so decoding and re-encoding a signed document changed its bytes.

One further departure, in the other direction: an uncertainty map holding a key
this version of the specification does not define is **rejected**
(`ERR_UNCERTAINTY_UNKNOWN_KEY`) rather than ignored. An uncertainty only partly
understood is not one to pass on as though it were understood entirely.
...
Neither the specification nor RFC 8949 defines these. The text grammar is this
project's own — [docs/text-format.md](text-format.md), frozen at 0.2.0 — and
the JSON profile is a mapping this project chose, described in the
[README](../README.md).
