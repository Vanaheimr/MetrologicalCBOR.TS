...
- **The property test was right all along: the fault is in `JSON.parse`.** Closed 2026-08-20. `tests/json/roundtrip.test.ts`, *the JSON survives being written and read as text*, failed twice under load with a shrunk counterexample that passed on replay; two million further executions found nothing; it was recorded as "not a function of its input" with the cause unknown. That last sentence was true of the wrong layer. The property is a function of its input. The platform underneath it is not.
  ...
  **What it is.** On Node 26.3.0, `JSON.parse` sometimes returns an object key one character short when the key had to be escaped. A key written `"\""` comes back as a lone backslash; `"*\""` comes back as `*\`. Every occurrence fits one description — the key holds the **raw** characters, cut to the length it would have had **unescaped**. Only the double quote shows it in these documents, because the only other escape they contain is `\\`, where the raw first character happens to be the right answer anyway.

  [scripts/v8-json-key-repro.mjs](scripts/v8-json-key-repro.mjs) demonstrates it in a hundred lines of plain JavaScript: no TypeScript, no library, no test runner, no dependency. That is deliberate. The first question anyone asks is whether the library is at fault, and answering that question is what the original investigation spent two days on.

  **What it takes, and what it is not.** Not a particular document: the text that failed parses correctly ten million times in a fresh process. It takes a long-lived process that has parsed a great many *freshly built* texts — a pool of texts prepared in advance and parsed in a loop does not do it, and neither does one text over and over. Given a seed it is nearly reproducible, usually failing at the same document; what decides is the history, and everything that changes the history moves the failure without removing it:
  ...
  **The compilers are not the cause**, and that was worth measuring rather than assuming, because assuming it cost half a day here too. The first flag runs — `--no-opt`, `--jitless`, `--no-maglev`, `--maglev-as-top-tier` — all came back green on the failing library seed, which reads exactly like a diagnosis. It was not one: those runs simply ended before the fault arrived. With no just-in-time compilation at all the fault is still there, later. Giving the young generation room enough to collect far less often moves it the same way, so the collector is somewhere in the mechanism — and a run that ends before the fault arrives proves nothing.

  **Why two million executions found nothing.** They were run in short processes and with clock-derived seeds, so nothing could be looked at twice. Both are now fixed, and both mattered: `reproducibly()` pins the seed, and `MCBOR_PROPERTY_RUNS` scales a run for a campaign. With `MCBOR_PROPERTY_SEED=83 MCBOR_PROPERTY_RUNS=5` the property fails at case 78 602 on demand, which is what turned this from a ghost into an afternoon. The rate is a rate per *process*, not per case:
  ...
  **What changed here.** `assertStable` no longer calls a repeating failure a counterexample. It repeats the computation, says so, and then says what that does and does not mean — because "it fails the same way twice" was read as "the input is the cause", and it is not: a process that has fallen into a fault stays in it and repeats just as faithfully. The remedy is one line long — replay the input in a fresh process — and it was never printed. On a failure the property now also asks the platform directly and says `THE PLATFORM LOST IT` where `JSON.parse` did not return what `JSON.stringify` wrote, which is conclusive when it fires; a clean answer does not clear the platform, because the fault comes and goes with the state of the process and that check runs after the fact.
