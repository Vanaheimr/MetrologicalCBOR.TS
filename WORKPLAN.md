...
3. **JSON → mCBOR detection default** — **decided 2026-08-22: `'none'`.** The default now guesses nothing: a string is a string until the caller names which strings are readings, by `'auto'` or by a predicate. The argument for `'auto'` was that the round trip then works without configuration; the argument against it is that it put the risk on the party who had not chosen, and the failure it risks is the invisible kind — a prose field holding `"1 h"` becomes a perfectly well-formed reading of something nobody measured, and nothing downstream can tell. What the flip cost was measured rather than guessed: **27 tests across five files**, among them the specification's own document vectors (`meter-reading-sorted`, `transaction-record-sorted`, `nested-reading`, which are marked `roundtrip: true`) and the conformance runner, which called `jsonTextToMcbor(jsonText)` bare and would have compared C#'s conversion against this library declining to perform one. All of them now say `readings: 'auto'` where they mean the conversion metrological-text.md Section 3 describes — which is the point: the conversion is a thing a caller asks for, and asking is visible in the source. This is a breaking change, and backwards compatibility was explicitly not a constraint at this stage of the project.
