...
- **A property test failed twice with a counterexample that was not one, and it is still unexplained.** `tests/json/roundtrip.test.ts`, *the JSON survives being written and read as text*, failed during two full runs under coverage. fast-check reported a shrunk counterexample; replaying it by seed and path **passes**, reconstructing the document by hand **passes**, and roughly **two million further executions** found nothing — 400 000 unbiased samples, 400 000 biased runs standalone, and 1 200 000 inside six further full runs under coverage. Ruled out along the way: the writer returning a view onto a reused buffer (it returns a copy), shared registry state (`withPrivateUnits` copies), non-ASCII or lone surrogates in the generated data (`fc.string` yields printable ASCII only), and member ordering (the encoder sorts, so order cannot matter).
  ...
  What is certain is that the property is **not a function of its input**, because a property-based tool shrinks towards whatever happened to fail — so an unstable execution is reported as a minimal counterexample that does not reproduce, which is the most misleading output a test can produce. The three properties there now go through `assertStable`, which repeats the computation before reporting and says which of the two it is, printing the document as hexadecimal so the next occurrence can be replayed directly instead of through the generator.

  **Investigated again on 2026-08-20. The cause was not found.** What changed is that three of the exclusions above are now measurements rather than claims, one hypothesis of its own was tested and refuted, and a real defect turned up — in the instrument rather than in the library.

  - *"`fc.string` yields printable ASCII only"* — **verified.** Twenty thousand samples under fast-check 4.9.0 contain no non-ASCII character, no astral pair and no lone surrogate. Worth checking rather than assuming, because fast-check 4 changed several generator defaults; this one did not move, so the exclusion stands and `-0`, NaN and lone surrogates remain out of reach of the generator.
  - *"only under coverage"* — **cannot be an instrumentation artefact.** The coverage provider is `v8`, which reads the inspector's counters and does not transform the source. Whatever coverage contributes, it contributes as load, memory or timing, not as different code.
  - *"the property depends on something outside its input"* — **not on library state.** Every module-level binding in `src/` that could hold state is a `Map` or `Set` built once from static data and never written to afterwards. There is no cache, no memo, no lazily-built index.
  - **A targeted search found nothing.** What separates this property from the two beside it that never failed is exactly one thing: it goes through `JSON.stringify`/`JSON.parse`. So rather than sample the document space a third time, 930 documents were enumerated over the input classes where a JavaScript object round trip is *not* the identity — array-index keys, keys that are names the language cares about, every ordered pair of them where enumeration order bites, and the same nested. All of them round-trip.
  - **`assertStable` did not ask its question when the computation threw.** It caught a wrong *answer* and re-ran, but let an exception straight through — so the one shape of failure where the reported input is least trustworthy was the shape it said nothing about. Fixed, with `tests/support/stability.test.ts` to keep it fixed: thirteen cases over agreeing, mismatching stably, mismatching unstably, mismatching differently each time, throwing stably, throwing unstably, and two throws that differ. Reverting the fix turns exactly the five throw cases red.

  **The properties are now seeded.** `fc.assert` defaults to a clock-derived seed, which is why the original failure could not be looked at twice: its inputs left with the process. `reproducibly(20_000)` pins the seed, so every push walks the same twenty thousand documents and a red run can be run again — and the nightly job sets `MCBOR_PROPERTY_SEED` to its own run number, so the exploring continues where nobody is waiting and any red night names the seed that did it. The failure message prints the seed alongside the input.

  **The fault is still open**, and it should still be treated as a known flake rather than as noise. What is different is that its next appearance will say which kind of failure it is, including if it arrives as an exception, and will be reproducible from the message alone.
